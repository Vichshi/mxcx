...
public class Swagger2Configuration implements WebMvcConfigurer {
	...
	public Docket createRestApi() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("v1").apiInfo(apiInfo())
				...
				.paths(PathSelectors.ant("/api/**")).build().enableUrlTemplating(false);
	}
	...
	private ApiInfo apiInfo() {
		...
		con.setName("vich");
		con.setUrl("");
		con.setEmail("xuan.955@163.com");
		return new ApiInfoBuilder().title("使用Swagger2构建RESTful APIs").description("简易后台管理--合同订单程序")
